...
**커밋 1개 = docs/ 작업 결과 문서 1개.** 커밋 단위 작업이 끝나면 반드시 해당 작업의 결과 문서를 작성한다.
...
- **문서는 쪼개지 않는다.** 그 커밋 단위의 모든 내용(작업 요약, 실행 명령, 변경 파일, 사용 버전, 선정/결정 이유, 알려진 이슈)을 한 파일에 담는다. 길어도 상관없다.
- 커밋을 찍을 때 해당 작업 결과 문서를 **같은 커밋에 포함**한다.
- 커밋은 사용자가 요청할 때만 찍는다. 문서는 작업 완료 시점에 미리 작성해 둔다.

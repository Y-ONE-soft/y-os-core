...
- **범위 중복 확인 (요청 사이클 시작 전 필수)**: `git worktree list`와 `gh pr list`로 진행 중인 브랜치·PR을 확인하고, 요청 범위가 겹치면 작업을 시작하지 말고 사용자에게 보고해 조정받는다. (실사례: DB 전환을 두 세션이 동시에 구현해 한쪽 전체 폐기 — docs/21)

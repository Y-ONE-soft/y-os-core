...
- docs 번호: 최초 43으로 작성 → 머지 직전 main에 43이 **2개**(PR #37 `43-style-…막대-이동-커서`, PR #38 `43-feat-작업-삭제-…`) 유입되어 본 문서를 **44**로 조정. **main에 남은 43 중복 2건은 이 브랜치에서 건드리지 않았다** — 별도 "문서 번호 중복 정리" 세션이 운영되고 있어(PR #36 선례) 동시 수정 시 충돌 위험이 있기 때문. 정리 대상으로 보고함

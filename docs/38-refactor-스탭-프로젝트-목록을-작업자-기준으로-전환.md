...
### 2차 리베이스 — PR #29·#30 반영 (충돌 해소)

추록 push 직후 main이 다시 `2ff02ce` → `84eb4bc`(PR #30)로 앞서가 재리베이스했고, 이번에는 **충돌이 발생했다.**

`src/components/features/projects/task-status-page.tsx`의 import 블록에서:

- main 쪽(PR #30 `chore: 작업 현황 헤더 ＋ 작업 버튼 제거`)이 `Button` import를 삭제
- 이 브랜치가 같은 블록의 `STAFF_ASSIGNED_PROJECT_IDS` import를 삭제

**양쪽 모두 삭제가 정답**이다. `Button`은 PR #30이 사용처를 없앴고, `STAFF_ASSIGNED_PROJECT_IDS`는 이 브랜치가 `ownerId` 필터로 대체했다. 두 심볼 모두 파일 내 다른 사용처가 없음을 확인한 뒤 두 줄을 함께 제거했다.

리베이스 후 재검증:

- `npm run build` 성공, `npm run lint` 경고 0
- step01 로그인 → `/`·`/projects`·`/projects/my-tasks` 모두 200
- 프로젝트 생성 후 "내 프로젝트" 목록에 즉시 반영 확인 (`YOS`, `Y.OS CONTENTS`, `최종 검증`)

문서 번호도 PR #30이 `docs/34`를 선점해 `34~36` → **`35~37`**로 재조정했다. 이 요청 사이클에서 번호를 두 번 올린 셈이다.

### 머지 후 프로덕션 배포 확인

- 머지 커밋 `24ce0a8`이 트리거한 **Production 배포 state = success**
- 프로덕션(`https://y-os-core.vercel.app`) 런타임 확인
  - `/api/health` → `{"status":"ok","db":"connected"}`
  - step01 로그인 200, 세션에 `groupId: g-soft` 포함 → **마이그레이션이 라이브에 반영됨**
  - 워크스페이스 응답에서 `ownerId` 기준 "내 프로젝트" 필터 정상 동작
- 프로덕션에 대한 **쓰기(프로젝트 생성) 검증은 수행하지 않았다.** 운영 환경에 데이터를 만드는 행위여서 자동 승인 정책에 의해 차단됐고 우회하지 않았다. 생성 경로는 로컬 dev에서 전 시나리오(위조 차단·그룹 미전송·마스터 회귀·비로그인 401)를 검증했고, 프로덕션 읽기 검증으로 스키마가 라이브에 반영된 것까지 확인했으므로 스키마 기인 실패 가능성은 없다.

### 문서 번호 3차 조정 (35~37 → 36~38)

머지 직전 PR #29가 자신의 문서를 `34→35`로 조정한 뒤 먼저 머지되어, main에 `docs/35`가 두 개 생겼다. "나중에 머지되는 쪽이 올린다"는 규칙에 따라 이 사이클의 문서를 한 칸씩 밀어 `36~38`로 정리했다(별도 소형 PR).

이 요청 사이클에서 문서 번호를 **세 번** 조정한 셈이다(`30~32` → `34~36` → `35~37` → `36~38`). 병렬 세션이 4개 이상 동시에 도는 상황에서는 번호 선점 경합이 반복되므로, 커밋 시점이 아니라 **머지 직전에 번호를 확정**하는 편이 조정 횟수를 줄인다.

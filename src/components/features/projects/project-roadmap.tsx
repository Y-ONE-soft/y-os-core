...
import { ROADMAP, STAGES } from "@/components/features/projects/project-detail-data";
  ...
  return (
    <section className="w-full shrink-0 rounded-[12px] border bg-background shadow-[0px_1px_3px_0px_rgba(0,0,0,0.05)]">
      ...
      <div className="relative">
        ...
        {STAGES.map((stage) => (
          <div key={stage.id} className="flex h-[30px] items-stretch border-t">
            ...
            <div className="relative min-w-0 flex-1">
              {stage.bar && (
                <div
                  ...
                  style={{
                    left: `${(stage.bar.startDay / ROADMAP.days) * 100}%`,
                    width: `${(stage.bar.days / ROADMAP.days) * 100}%`,
                    backgroundColor: stage.bar.bg,
                    borderColor: stage.bar.border,
                  }}
                >
                  <span
                    className="whitespace-nowrap text-[10.5px] font-medium"
                    style={{ color: stage.bar.text }}
                  >
                    {stage.bar.label}
                  </span>
                </div>
